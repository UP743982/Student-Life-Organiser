...
const node_connect_db = mongoose.connect("mongodb://localhost/port_connect");
...
io.on("connection", function(socket){
	...
	socket.on("message_from_client", function(usr_msg){
		console.log(usr_msg);
		let all_connected_clients = io.sockets.connected;
		for(let socket_id in all_connected_clients){
			if(all_connected_clients[socket_id].member_id === usr_msg.friend_member_id){
				let message_object = {"msg": usr_msg.msg, "user_name": socket.user_name};
