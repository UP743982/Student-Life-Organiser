				...
				handler: function(request, reply){
					User.find({"email": request.auth.credentials.user}, function(err,sending_user){
						User.find({"member_id": request.payload.friend_member_id}, function(err, potential_friend){
							potential_friend[0].update({$push: {"friends_request": {"member_id": sending_user[0].member_id, "friend_name": sending_user[0].name, "profile_pic": sending_user[0].user_profile[0].profile_pic}}}, function(err){
								reply();
							});
						});
					});
				...
				handler: function(request, reply){
					User.find({"email": request.auth.credentials.user}, function(err, user){
						...
						User.find({"member_id": request.payload.member_id}, function(err, accepted_friend_user){
							user[0].update({$push: {"friends": {"member_id": accepted_friend_user[0].member_id, "friend_name": accepted_friend_user[0].name, "profile_pic": accepted_friend_user[0].user_profile[0].profile_pic}}, $pull: {"friends_request": {member_id: request.payload.member_id}}},
								function(err){
